...
public class MainController implements Observer {
    ...
    @FXML
    private TableView<FriendRequest> tableFriendsRequestSent;
    @FXML
    private TableColumn<FriendRequest, String> requestTo;
    @FXML
    private TableColumn<FriendRequest, String> requestStatus1;
    @FXML
    private TableColumn<FriendRequest, String> requestDate1;
    ...
    private void initReceivedTableModel() {
        List<FriendRequest> friendRequests = friendRequestService.getAllFriendRequestsOfAUser(user.getId());
        receivedTableModel.setAll(friendRequests);
    }

    private void initializeSentTable() {
        requestTo.setCellValueFactory(new PropertyValueFactory<>("StringTo"));
        requestStatus1.setCellValueFactory(new PropertyValueFactory<>("Status"));
        requestDate1.setCellValueFactory(new PropertyValueFactory<>("StringDate"));
        tableFriendsRequestSent.setItems(sentTableModel);
    }

    private void initSentTableModel() {
        List<FriendRequest> friendRequests = friendRequestService.getAllFriendRequestsSentByAUser(user.getId());
    ...
    @FXML
    public void handleUnsentRequest() {
        FriendRequest selectedRequest = tableFriendsRequestSent.getSelectionModel().getSelectedItem();
        if (selectedRequest != null) {
            FriendRequest unsentRequest = friendRequestService.unsentRequest(selectedRequest.getId());
            if (unsentRequest == null)
                showMessage(
                        null, Alert.AlertType.INFORMATION, "Unsent", "You have unsent the friend request to " + selectedRequest.getTo()+"!"
                );
        }
        else {
            showErrorMessage(null, "You must select a request!");
